/**
 * Sistema de Calificación de Leads - Tributos al Alcance (VZLA)
 * Diseñado para integración con WordPress tributosatualcance.org
 * 
 * @version 1.0.0
 * @author Growth Hacking - Captador VIP
 */

// ============================================================================
// CONSTANTES - Mapeo del Cuestionario (Normativa SENIAT Venezuela)
// ============================================================================

const TIPO_SUJETO = {
  PERSONA_NATURAL: 'persona_natural',
  PYME: 'pyme',
  CONTRIBUYENTE_ESPECIAL: 'contribuyente_especial'
};

const UBICACION = {
  CARACAS: 'caracas',
  VALENCIA: 'valencia',
  MARACAIBO: 'maracaibo',
  RESTO_PAIS: 'resto_pais'
};

const DOLOR_PRINCIPAL = {
  MULTAS_SENIAT: 'multas_seniat',
  MANEJO_IGTF: 'manejo_igtf',
  RETENCIONES_IVA_ISLR: 'retenciones_iva_islr',
  AUDITORIA_PREVENTIVA: 'auditoria_preventiva'
};

const RANGOS_INGRESOS = {
  MENOR_1000: { min: 0, max: 1000 },
  ENTRE_1000_5000: { min: 1000, max: 5000 },
  ENTRE_5000_20000: { min: 5000, max: 20000 },
  MAYOR_20000: { min: 20000, max: Infinity }
};

const NIVEL_RIESGO = {
  BAJO: 'bajo',
  MEDIO: 'medio',
  ALTO: 'alto',
  CRITICO: 'critico'
};

// ============================================================================
// CRITERIOS VIP (Venezuela - SENIAT)
// ============================================================================
const UMBRAL_INGRESOS_VIP = 5000;

// ============================================================================
// VALIDACIÓN RIF (Registro de Información Fiscal - Venezuela SENIAT)
// ============================================================================

const RIF_LETRAS_VALIDAS = ['V', 'E', 'J', 'P', 'G'];
const RIF_VALORES_LETRA = { V: 4, E: 8, J: 12, P: 16, G: 20 };
const RIF_FACTORES = [0, 3, 2, 7, 6, 5, 4, 3, 2];

/**
 * Valida el RIF venezolano según normativa SENIAT.
 * Formato: X-XXXXXXXX-X (ej: V-12345678-9, J-00012345-1)
 * Letras: V (venezolano), E (extranjero), J (jurídica), P (pasaporte), G (gobierno)
 * @param {string} rif - RIF con o sin guiones (ej: V123456789 o V-12345678-9)
 * @returns {{ valido: boolean, rifFormateado?: string, mensaje?: string }}
 */
function validarRIF(rif) {
  if (!rif || typeof rif !== 'string') {
    return { valido: false, mensaje: 'El RIF es obligatorio' };
  }

  const normalizado = rif.replace(/\s|-/g, '').toUpperCase();
  if (normalizado.length < 9 || normalizado.length > 10) {
    return { valido: false, mensaje: 'El RIF debe tener formato X-XXXXXXXX-X (letra + 8 dígitos + dígito verificador)' };
  }

  const letra = normalizado[0];
  if (!RIF_LETRAS_VALIDAS.includes(letra)) {
    return { valido: false, mensaje: `La letra del RIF debe ser: V, E, J, P o G (recibido: ${letra})` };
  }

  const digitos = normalizado.slice(1, 9);
  if (!/^\d{8}$/.test(digitos)) {
    return { valido: false, mensaje: 'Los 8 dígitos centrales del RIF deben ser numéricos' };
  }

  let suma = 0;
  suma += RIF_VALORES_LETRA[letra];
  for (let i = 0; i < 8; i++) {
    suma += RIF_FACTORES[i + 1] * parseInt(digitos[i], 10);
  }

  let digitoVerificador = 11 - (suma % 11);
  if (digitoVerificador >= 10) digitoVerificador = 0;

  const rifEsperado = letra + digitos + digitoVerificador;
  const rifIngresado = normalizado.length === 10 ? normalizado : null;

  if (rifIngresado && rifIngresado !== rifEsperado) {
    return { valido: false, mensaje: `Dígito verificador inválido. El RIF correcto sería: ${formatearRIF(rifEsperado)}` };
  }

  return {
    valido: true,
    rifFormateado: formatearRIF(rifEsperado)
  };
}

function formatearRIF(rif) {
  if (!rif || rif.length < 9) return rif;
  const limpio = rif.replace(/\s|-/g, '').toUpperCase();
  return `${limpio[0]}-${limpio.slice(1, 9)}-${limpio.slice(-1)}`;
}

// ============================================================================
// BASE DE DATOS MOCK (Reemplazable por SQLite vía MCP o MySQL para WordPress)
// ============================================================================

const mockDatabase = {
  leads: [],
  
  save(lead) {
    const record = {
      id: `lead_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      ...lead,
      createdAt: new Date().toISOString()
    };
    this.leads.push(record);
    return record;
  },
  
  findById(id) {
    return this.leads.find(l => l.id === id) || null;
  },
  
  findAll() {
    return [...this.leads];
  }
};

// ============================================================================
// PROCESAMIENTO DEL CUESTIONARIO - 6 Preguntas
// ============================================================================

/**
 * Procesa las respuestas del cuestionario y calcula el nivel de riesgo
 * @param {Object} respuestas - Objeto con las 6 respuestas del formulario
 * @returns {Object} Resultado con score, nivel de riesgo y metadatos
 */
function procesarCuestionario(respuestas) {
  const {
    tipoSujeto,
    ubicacion,
    ingresosMensuales, // Valor numérico en USD
    dolorPrincipal,
    maquinasFiscalesActualizadas, // boolean
    fiscalizacionActiva // boolean
  } = respuestas;

  if (!tipoSujeto || !ubicacion || ingresosMensuales === undefined) {
    throw new Error('Faltan campos obligatorios del cuestionario');
  }

  let scoreRiesgo = 0; // Mayor score = mayor riesgo
  const factores = [];

  // Q1: Tipo de Sujeto - Contribuyente Especial = mayor exposición tributaria
  if (tipoSujeto === TIPO_SUJETO.CONTRIBUYENTE_ESPECIAL) {
    scoreRiesgo += 35;
    factores.push('Contribuyente Especial (alta exposición SENIAT)');
  } else if (tipoSujeto === TIPO_SUJETO.PYME) {
    scoreRiesgo += 20;
    factores.push('Pyme (obligaciones tributarias medias)');
  } else {
    scoreRiesgo += 5;
    factores.push('Persona Natural (menor complejidad)');
  }

  // Q2: Ubicación - Solo para logística, impacto menor en riesgo
  if (ubicacion === UBICACION.CARACAS) {
    scoreRiesgo += 5; // Mayor presencia SENIAT
  }
  factores.push(`Ubicación: ${ubicacion}`);

  // Q3: Ingresos - Mayor volumen = mayor exposición
  if (ingresosMensuales >= 20000) {
    scoreRiesgo += 30;
    factores.push('Ingresos altos (>$20k - cuenta grande)');
  } else if (ingresosMensuales >= 5000) {
    scoreRiesgo += 20;
    factores.push('Ingresos medios-altos ($5k-$20k)');
  } else if (ingresosMensuales >= 1000) {
    scoreRiesgo += 10;
    factores.push('Ingresos medios ($1k-$5k)');
  } else {
    scoreRiesgo += 2;
    factores.push('Ingresos bajos (<$1k)');
  }

  // Q4: Dolor Principal - Indica problemas existentes
  switch (dolorPrincipal) {
    case DOLOR_PRINCIPAL.MULTAS_SENIAT:
      scoreRiesgo += 25;
      factores.push('Multas del SENIAT (problema activo)');
      break;
    case DOLOR_PRINCIPAL.MANEJO_IGTF:
      scoreRiesgo += 15;
      factores.push('Manejo de IGTF');
      break;
    case DOLOR_PRINCIPAL.RETENCIONES_IVA_ISLR:
      scoreRiesgo += 20;
      factores.push('Retenciones IVA/ISLR');
      break;
    case DOLOR_PRINCIPAL.AUDITORIA_PREVENTIVA:
      scoreRiesgo += 10;
      factores.push('Auditoría preventiva (proactivo)');
      break;
    default:
      scoreRiesgo += 5;
  }

  // Q5: Máquinas Fiscales - No actualizadas = riesgo alto
  if (!maquinasFiscalesActualizadas) {
    scoreRiesgo += 25;
    factores.push('Máquinas fiscales NO actualizadas (riesgo de sanciones)');
  } else {
    factores.push('Máquinas fiscales actualizadas');
  }

  // Q6: Fiscalización Activa - URGENCIA MÁXIMA
  if (fiscalizacionActiva) {
    scoreRiesgo += 40; // Peso muy alto - situación crítica
    factores.push('⚠️ FISCALIZACIÓN ACTIVA - Urgencia máxima');
  } else {
    factores.push('Sin fiscalización activa');
  }

  // Normalizar score (0-100) y determinar nivel
  const scoreNormalizado = Math.min(100, scoreRiesgo);
  let nivelRiesgo;

  if (scoreNormalizado >= 70) nivelRiesgo = NIVEL_RIESGO.CRITICO;
  else if (scoreNormalizado >= 50) nivelRiesgo = NIVEL_RIESGO.ALTO;
  else if (scoreNormalizado >= 30) nivelRiesgo = NIVEL_RIESGO.MEDIO;
  else nivelRiesgo = NIVEL_RIESGO.BAJO;

  return {
    scoreRiesgo: scoreNormalizado,
    nivelRiesgo,
    factores,
    respuestas
  };
}

// ============================================================================
// checkVipStatus() - Alerta según criterios Venezuela (SENIAT)
// ============================================================================

/**
 * Determina si el lead es VIP según normativa SENIAT Venezuela.
 * Criterios: Contribuyente Especial, Ingresos >$5k, o Fiscalización activa.
 * @param {Object} resultado - Resultado de procesarCuestionario()
 * @returns {Object} { esVip: boolean, razones: string[] }
 */
function checkVipStatus(resultado) {
  const razones = [];
  const { respuestas } = resultado;

  if (respuestas.tipoSujeto === TIPO_SUJETO.CONTRIBUYENTE_ESPECIAL) {
    razones.push('Es Contribuyente Especial (normativa SENIAT)');
  }

  if (respuestas.ingresosMensuales > UMBRAL_INGRESOS_VIP) {
    razones.push(`Ingresos mensuales > $${UMBRAL_INGRESOS_VIP.toLocaleString()} ($${respuestas.ingresosMensuales?.toLocaleString()})`);
  }

  if (respuestas.fiscalizacionActiva) {
    razones.push('Tiene fiscalización activa - Urgencia máxima');
  }

  return {
    esVip: razones.length > 0,
    razones
  };
}

// ============================================================================
// NOTIFICACIÓN VIP - Slack Webhook / Console
// ============================================================================

/**
 * Envía alerta cuando se detecta cliente VIP.
 * Soporta Slack webhook o console.log detallado.
 * @param {Object} params - { nombre, telefono, razones }
 * @param {Object} config - { slackWebhookUrl?: string }
 */
async function notificarClienteVip(params, config = {}) {
  const { nombre, telefono, razones } = params;
  const razonTexto = razones.join(' | ');

  const mensaje = `🚨 CLIENTE VIP DETECTADO: ${nombre || 'Sin nombre'} - ${razonTexto}`;
  const payload = {
    timestamp: new Date().toISOString(),
    nombre,
    telefono,
    razones,
    mensaje
  };

  // Slack Webhook (si está configurado)
  if (config.slackWebhookUrl) {
    try {
      const response = await fetch(config.slackWebhookUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text: mensaje,
          blocks: [
            {
              type: 'section',
              text: { type: 'mrkdwn', text: `*🚨 CLIENTE VIP DETECTADO*` }
            },
            {
              type: 'section',
              fields: [
                { type: 'mrkdwn', text: `*Nombre:*\n${nombre || 'N/A'}` },
                { type: 'mrkdwn', text: `*Teléfono:*\n${telefono || 'N/A'}` },
                { type: 'mrkdwn', text: `*Razón VIP:*\n${razonTexto}` }
              ]
            }
          ]
        })
      });
      if (!response.ok) throw new Error(`Slack HTTP ${response.status}`);
    } catch (err) {
      console.error('[notificarClienteVip] Error Slack:', err.message);
    }
  }

  // Console detallado (siempre)
  console.log('\n' + '═'.repeat(60));
  console.log('🚨 CLIENTE VIP DETECTADO:', nombre || 'Sin nombre');
  console.log('   Razón:', razonTexto);
  console.log('   Teléfono:', telefono || 'N/A');
  console.log('   Timestamp:', payload.timestamp);
  console.log('═'.repeat(60) + '\n');

  return payload;
}

// ============================================================================
// FLUJO PRINCIPAL - Registrar Lead y Evaluar VIP
// ============================================================================

/**
 * Registra un lead, lo califica y dispara alerta si es VIP.
 * Requiere RIF válido antes de completar el registro.
 * Integrable con REST API de WordPress.
 * @param {Object} leadData - { nombre, telefono, rif, ...respuestas }
 * @param {Object} config - { slackWebhookUrl?, db? }
 * @returns {Object} Lead guardado con diagnóstico
 * @throws {Error} Si el RIF es inválido o falta
 */
async function registrarLead(leadData, config = {}) {
  const { nombre, telefono, rif, ...respuestas } = leadData;
  const db = config.db || mockDatabase;

  const validacionRIF = validarRIF(rif);
  if (!validacionRIF.valido) {
    throw new Error(validacionRIF.mensaje || 'RIF inválido');
  }

  const resultado = procesarCuestionario(respuestas);
  const vipStatus = checkVipStatus(resultado);

  const record = db.save({
    nombre: nombre || 'Anónimo',
    telefono: telefono || '',
    rif: validacionRIF.rifFormateado,
    nivelRiesgo: resultado.nivelRiesgo,
    scoreRiesgo: resultado.scoreRiesgo,
    esVip: vipStatus.esVip,
    factores: resultado.factores,
    respuestas
  });

  if (vipStatus.esVip) {
    await notificarClienteVip(
      {
        nombre: record.nombre,
        telefono: record.telefono,
        razones: vipStatus.razones
      },
      { slackWebhookUrl: config.slackWebhookUrl }
    );
  }

  return {
    ...record,
    vipStatus,
    diagnostico: {
      nivelRiesgo: resultado.nivelRiesgo,
      scoreRiesgo: resultado.scoreRiesgo,
      factores: resultado.factores
    }
  };
}

// ============================================================================
// EXPORTS (Node.js / WordPress REST / Build)
// ============================================================================

if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    validarRIF,
    procesarCuestionario,
    checkVipStatus,
    notificarClienteVip,
    registrarLead,
    mockDatabase,
    TIPO_SUJETO,
    UBICACION,
    DOLOR_PRINCIPAL,
    RANGOS_INGRESOS,
    NIVEL_RIESGO,
    UMBRAL_INGRESOS_VIP
  };
}
